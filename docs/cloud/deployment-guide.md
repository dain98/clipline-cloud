...
This is the friendly, step-by-step guide to deploying Clipline Cloud v1.2.23 with Docker Compose. For the
...
```text
CLIPLINE_IMAGE=ghcr.io/dain98/clipline-cloud:1.2.23
```
...
CLIPLINE_IMAGE=ghcr.io/dain98/clipline-cloud:1.2.23 docker compose up -d
...
container must be readable by UID `10001`, for example with
...
The `clipline-cloud` container reads `database_url.txt`, `admin_password.txt`, and
`session_secret.txt`, so those files must be readable by UID `10001`.
...
The `clipline-cloud` container reads these secret files, so they must be readable
by UID `10001`.
...
```sh
BUILD_IMAGE=0 CLIPLINE_IMAGE=ghcr.io/dain98/clipline-cloud:1.2.23 deploy/compose/smoke.sh
```
