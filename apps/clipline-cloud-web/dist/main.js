...
`)),k(`Copied ${P.length} link${P.length===1?"":"s"}`+(I?` (${I} skipped, private)`:""))}catch{k("Couldn't copy links to clipboard.")}}async function me(){let $=Array.from(p);w(!1);try{let T=await y("/api/v1/clips/bulk-delete",{method:"POST",body:{ids:$}});b(new Set),J(),k(`Deleted ${T.affected} clip${T.affected===1?"":"s"}.`)}catch(T){k(T.message)}}if(d)return o`<main class="page">
  ...
  </div>`}Q();var bs={ChampionKill:"kill",FirstBlood:"kill",Multikill:"spree",Ace:"spree",DragonKill:"objective",HeraldKill:"objective",BaronKill:"objective",TurretKilled:"structure",InhibKilled:"structure",FirstBrick:"structure"};function Zn(e){let t=Number(e);return Number.isFinite(t)&&t>0?t/1e3:0}function Jn(e,t){let n=Number.isFinite(e)?e:0,a=t>0?t:Number.MAX_SAFE_INTEGER;return Math.max(0,Math.min(a,n))}function rt(e,t){return t>0?Math.max(0,Math.min(100,e/t*100)):0}function Pt(e){if(!Number.isFinite(e))return"0:00";let t=Math.max(0,Math.round(e)),n=Math.floor(t/60),a=t-n*60;return`${n}:${String(a).padStart(2,"0")}`}function jn(e){if(!Number.isFinite(e))return"0:00.0";let t=Math.max(0,Math.round(e*10)),n=Math.floor(t/600),a=t-n*600,r=Math.floor(a/10);return`${n}:${String(r).padStart(2,"0")}.${a%10}`}function Yn(e,t){return`${jn(e)} / ${t>0?jn(t):"0:00.0"}`}function vs(e){return bs[e]||"info"}function Xn(e,t){return(e||[]).map((n,a)=>{let r=Number(n.timestamp_ms);if(!Number.isFinite(r))return null;let s=r/1e3;return s<0||t>0&&s>t?null:{index:a,time:s,kind:String(n.kind||"Marker"),label:String(n.label||n.kind||"Marker"),category:vs(n.kind)}}).filter(Boolean).sort((n,a)=>n.time-a.time)}function Qn(e,t){if(!e.length)return null;for(let n of e)if(n.time>t+.05)return n;return e[0]}function ea(e,t){if(!e.length)return null;for(let n=e.length-1;n>=0;n-=1)if(e[n].time<t-.05)return e[n];return e[e.length-1]}function ta(e,t){switch(e){case"Space":case"KeyK":return{kind:"toggle-play"};case"ArrowLeft":return{kind:"seek-by",seconds:t?-1:-5};case"ArrowRight":return{kind:"seek-by",seconds:t?1:5};case"KeyJ":return{kind:"seek-by",seconds:-10};case"KeyL":return{kind:"seek-by",seconds:10};case"Comma":return{kind:"seek-by",seconds:-.1};case"Period":return{kind:"seek-by",seconds:.1};case"KeyM":return{kind:t?"previous-marker":"next-marker"};case"Home":return{kind:"seek-to",seconds:0};case"End":return{kind:"seek-to-end"};case"KeyF":return{kind:"fullscreen"};case"KeyT":return{kind:"theater"};default:return null}}var aa="clipline.playerVolume",sa="clipline.clipTheaterMode",$s=2e3,gs=[.25,.5,.75,1,1.25,1.5,2];function ys(e,t){switch(e){case"KeyM":return{kind:"toggle-mute"};case"KeyF":return{kind:"theater"};case"Escape":return{kind:"exit-theater"};default:return ta(e,t)}}function ks(e){return e instanceof Element?!!e.closest("input, textarea, select, button, a, [contenteditable='true'], [contenteditable='']"):!1}function ws(){try{let e=window.localStorage.getItem(aa);if(e==null)return 1;let t=Number(e);return Number.isFinite(t)?Math.max(0,Math.min(1,t)):1}catch{return 1}}function na(e){try{window.localStorage.setItem(aa,String(Math.max(0,Math.min(1,e))))}catch{}}function Ss(){try{return window.localStorage.getItem(sa)==="true"}catch{return!1}}function xs(e){try{window.localStorage.setItem(sa,String(e))}catch{}}function ra({src:e,poster:t,durationMs:n,markers:a}){let r=z(null),s=z(null),i=z(null),c=z(!1),d=z(!1),u=Zn(n),[p,b]=_(!1),[l,h]=_(0),[m,w]=_(u),[x,C]=_(0),[g,R]=_(ws),[j,J]=_(!1),[Z,se]=_(1),[V,te]=_(!1),[L,Y]=_(Ss),[ke,ne]=_(!0),[be,ve]=_(null),[pe,ce]=_(""),me=Xn(a,m);function X(){ne(!0),window.clearTimeout(i.current),i.current=window.setTimeout(()=>{let f=r.current;f&&!f.paused&&!f.ended&&ne(!1)},$s)}S(()=>{p||(window.clearTimeout(i.current),ne(!0))},[p]),S(()=>{let f=r.current;if(!f)return;let E=()=>Number.isFinite(f.duration)&&f.duration>0?f.duration:u,F=()=>w(E()),Ut=()=>w(E()),Lt=()=>{c.current||h(f.currentTime||0)},It=()=>{let Vt=E();if(!(Vt>0)||!f.buffered?.length){C(0);return}let Ot=f.currentTime||0,Ue=0;for(let Le=0;Le<f.buffered.length;Le+=1){let Ta=f.buffered.start(Le),ct=f.buffered.end(Le);if(Ot>=Ta&&Ot<=ct){Ue=ct;break}Ue=Math.max(Ue,ct)}C(rt(Ue,Vt))},At=()=>{b(!0),ce(""),X()},Nt=()=>b(!1),Ft=()=>b(!1),zt=()=>{R(f.volume),J(f.muted||f.volume===0)},Bt=()=>ce("Playback unavailable");return f.addEventListener("loadedmetadata",F),f.addEventListener("durationchange",Ut),f.addEventListener("timeupdate",Lt),f.addEventListener("progress",It),f.addEventListener("play",At),f.addEventListener("pause",Nt),f.addEventListener("ended",Ft),f.addEventListener("volumechange",zt),f.addEventListener("error",Bt),()=>{f.removeEventListener("loadedmetadata",F),f.removeEventListener("durationchange",Ut),f.removeEventListener("timeupdate",Lt),f.removeEventListener("progress",It),f.removeEventListener("play",At),f.removeEventListener("pause",Nt),f.removeEventListener("ended",Ft),f.removeEventListener("volumechange",zt),f.removeEventListener("error",Bt)}},[e,u]),S(()=>{r.current&&(r.current.volume=g)},[g]),S(()=>{r.current&&(r.current.muted=j)},[j]),S(()=>{r.current&&(r.current.playbackRate=Z)},[Z]),S(()=>{let f=document.documentElement;return f.classList.toggle("clipline-theater",L),()=>f.classList.remove("clipline-theater")},[L]);function re(f){Y(f),xs(f)}function oe(f){let E=r.current;if(!E)return;let F=m>0?Jn(f,m):Math.max(0,f);E.currentTime=F,h(F)}function $e(f){oe((r.current?.currentTime||0)+f)}async function ge(){let f=r.current;if(f)if(f.paused||f.ended)try{await f.play()}catch(E){ce(E?.message||"Playback failed")}else f.pause()}function we(){let f=r.current;f&&(f.muted||f.volume===0?(f.muted=!1,f.volume===0&&(f.volume=1,R(1),na(1)),J(!1)):(f.muted=!0,J(!0)))}function $(f){let E=Number(f.target.value);R(E),J(E===0),na(E);let F=r.current;F&&(F.volume=E,F.muted=E===0)}async function T(){try{document.fullscreenElement?await document.exitFullscreen():await s.current?.requestFullscreen?.()}catch(f){ce(f?.message||"Fullscreen unavailable")}}function v(f){let E=r.current?.currentTime||0,F=f>0?Qn(me,E):ea(me,E);F&&oe(F.time)}function P(){c.current=!0,d.current=p,p&&r.current?.pause()}function I(f){let E=Number(f.target.value);h(E),oe(E)}function D(){c.current&&(c.current=!1,d.current&&(d.current=!1,r.current?.play().catch(()=>{})))}function U(f){let E=f.currentTarget.getBoundingClientRect();if(!(E.width>0))return;let F=Math.max(0,Math.min(1,(f.clientX-E.left)/E.width));ve({pct:F*100,time:F*(m||0)})}function ae(){ve(null)}return S(()=>{function f(E){if(E.defaultPrevented||ks(E.target))return;let F=ys(E.code,E.shiftKey);if(F&&!(F.kind==="exit-theater"&&!L))switch(E.preventDefault(),X(),F.kind){case"toggle-play":ge();break;case"seek-by":$e(F.seconds);break;case"seek-to":oe(F.seconds);break;case"seek-to-end":oe(m);break;case"next-marker":v(1);break;case"previous-marker":v(-1);break;case"toggle-mute":we();break;case"theater":re(!L);break;case"exit-theater":re(!1);break;case"fullscreen":T();break;default:break}}return document.addEventListener("keydown",f),()=>document.removeEventListener("keydown",f)},[m,L,p]),o`<div class=${`player ${ke?"":"chrome-hidden"}`} ref=${s}
    ...
    <div class="player-overlay">
      <div class="player-timeline" onPointerMove=${U} onPointerLeave=${ae}>
        ...
        ${me.map(f=>o`<span class="player-marker-tick" key=${f.index}
            style=${`left:${rt(f.time,m)}%`} title=${`${f.label} @ ${Pt(f.time)}`}></span>`)}
  ...
  </main>`}var lr={publicLibrary:et,publicGame:et,games:Vn,library:Wn,clip:Et,public:Et,login:ca,resetPassword:ua,admin:va,profile:ya,account:ka,publicUser:wa,about:xa},Ca=Tn({pathname:window.location.pathname,search:window.location.search});function cr(){let e=Pn();Ca=e.name;let{ready:t,user:n}=H(A),a=t&&xn(e.name,n);if(S(()=>{a&&G("/login")},[a]),!t||a)return o`<div class="boot">Loading…</div>`;let r=lr[e.name]||et,s=e.name==="login"||e.name==="resetPassword";return o`<div class="ui" onClick=${En}>
    ${!s&&o`<${Dn} active=${wt(e)} route=${e} />`}
    <${r} route=${e} />
    ${!s&&o`<${Un} active=${Cn(e)} />`}
    <${Ln} />
  </div>`}window.addEventListener("clipline:unauthorized",()=>{de(null),A.set({user:null,csrfToken:null,ready:!0}),kt(Ca)||G("/login")});(async()=>{try{let t=await y("/api/v1/auth/me");de(t.csrf_token),A.set({user:t.user,csrfToken:t.csrf_token,ready:!0})}catch{de(null),A.set({user:null,csrfToken:null,ready:!0})}let e=document.querySelector("#app");e.textContent="",sn(o`<${cr} />`,e)})();
