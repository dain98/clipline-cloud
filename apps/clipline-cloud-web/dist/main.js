    ...
    ${c==null?o`<${is} />`:c.length===0?o`<${W} name="film"
        ...
        <div class="feed-toolbar">
          ...
          <select class="input" value=${t.sort} onChange=${m=>p({sort:m.target.value})}>
            ${ts.map(([m,g])=>o`<option value=${m}>${g}</option>`)}
          </select>
          <div class="chips">
...
`)),k(`Copied ${E.length} link${E.length===1?"":"s"}`+(A?` (${A} skipped, private)`:""))}catch{k("Couldn't copy links to clipboard.")}}async function me(){let $=Array.from(d);g(!1);try{let P=await w("/api/v1/clips/bulk-delete",{method:"POST",body:{ids:$}});h(new Set),Z(),k(`Deleted ${P.affected} clip${P.affected===1?"":"s"}.`)}catch(P){k(P.message)}}if(p)return o`<main class="page">
    ...
    <div class="player-overlay">
      ...
      <div class="player-controls">
        ...
        <input class="player-volume" type="range" min="0" max="1" step="0.01" value=${j?0:y}
    ...
    ${n==null?"":T.count===0?o`<p class="comment-signin">No comments yet.</p>`:o`<div class="comment-list">
          ${T.roots.map(y=>ca(y,{depth:0,repliesByParent:T.repliesByParent,user:t,replyOpenId:i,setReplyOpenId:u,replyDraft:p,setReplyDraft:c,submitReply:g,onDelete:h}))}
        </div>`}
    ...
    <div>
      ...
      <div class="watch-desc">
        ${l?o`<textarea class="input" rows="5" value=${m} autofocus
              onInput=${v=>g(v.target.value)} onBlur=${ie}
  ...
  </${Pe}>`}function pa({route:e}){let t=!!e.invite,[n,a]=b(()=>t?"preflight":e.token?"form":"missing-token"),[r,s]=b(""),[i,u]=b(t?null:e.token),[p,c]=b(""),[d,h]=b(!1),l=t;x(()=>{if(!t)return;if(!e.token){a("missing-token");return}let S=!0;return a("preflight"),w("/api/v1/invites/claim",{method:"POST",body:{invite_token:e.token}}).then(T=>{S&&(u(T.reset_token),a("form"))}).catch(T=>{S&&(s(T instanceof _e?T.message:"This invite link is invalid, used, or expired."),a("invalid"))}),()=>{S=!1}},[t,e.token]);async function f(S){if(S.preventDefault(),d)return;h(!0),c("");let T=new FormData(S.currentTarget),y={reset_token:i,new_password:String(T.get("new_password")||"")};l&&(y.username=String(T.get("username")||""),y.display_name=ua(T.get("display_name")),y.email=ua(T.get("email")));try{await w("/api/v1/auth/reset-password",{method:"POST",body:y}),k(l?"Account created. Sign in with your new password.":"Password set. Sign in with your new password."),G("/login")}catch(R){c(R instanceof _e?R.message:"Request failed"),h(!1)}}if(t&&n!=="form"){let S=n==="missing-token"||n==="invalid",T=n==="missing-token"?"This invite link is missing a token.":n==="invalid"?r:"Opening invite\u2026";return o`<${Pe} titleId="invite-title">
      <h1 id="invite-title">Create account</h1>
      <p class="login-copy">${S?"This invite cannot be used.":"Preparing your account setup."}</p>
      ${S?o`<p class="form-error" role="alert">${T}</p>`:o`<p class="login-status">${T}</p>`}
    ...
    <td>
      ${l?o`<select class="input input-compact" value=${f}
            onChange=${g=>{let S=g.target.value;S!==e.role&&(m(e.role),i(e,S))}}>
            ${Lt(!0).map(([g,S])=>o`<option value=${g} selected=${f===g}>${S}</option>`)}
          </select>`:e.role}
    </td>
    <td>${Xs(e)}</td>
    <td>
  ...
  </tr>`}function fa({users:e,settings:t,currentUser:n,resetLink:a,setResetLink:r,reload:s}){let[i,u]=b(null),p=n?.role==="owner",c=!!t?.smtp_enabled,d=()=>u(null);async function h(){let{type:f,user:m,value:g}=i;d();try{if(f==="quota"){let S=g.trim()?It(g):null;await w(`/api/v1/users/${encodeURIComponent(m.id)}`,{method:"PATCH",body:{storage_quota_bytes:S}}),k("Storage quota updated.")}else if(f==="disable")await w(`/api/v1/users/${encodeURIComponent(m.id)}`,{method:"DELETE",body:{reauth_password:g}}),k("User disabled.");else if(f==="enable")await w(`/api/v1/users/${encodeURIComponent(m.id)}`,{method:"PATCH",body:{is_disabled:!1,reauth_password:g}}),k("User enabled.");else if(f==="role")await w(`/api/v1/users/${encodeURIComponent(m.id)}`,{method:"PATCH",body:{role:g.role,reauth_password:g.password}}),k(`Role updated to ${g.role}.`);else if(f==="purge")await w(`/api/v1/users/${encodeURIComponent(m.id)}/purge`,{method:"POST",body:{reauth_password:g}}),k("User deleted.");else if(f==="reset"){let S=await w(`/api/v1/users/${encodeURIComponent(m.id)}/reset-password`,{method:"POST",body:{reauth_password:g}});r({...S,kind:"reset"}),k("Reset link created.")}s()}catch(S){k(S.message),s()}}let l={quota:{title:"Set storage quota",description:"Enter a per-user storage limit in GiB. Leave it blank to remove the per-user limit.",confirmLabel:"Save quota",danger:!1,field:o`<label class="field"><span>Quota GiB</span>
        ...
        <table class="lib-table">
          ...
          <tbody>
            ${e.map(f=>o`<${er} key=${f.id} user=${f} currentUser=${n}
              ...
              onRole=${(m,g)=>u({type:"role",user:m,value:{role:g,password:""}})}
  ...
  </main>`:null}X();function ya(e){let t=String(e||"").trim();return t||null}async function or(e){let t=new Headers;t.set("Accept","application/json"),t.set("Content-Type",e.type||"application/octet-stream");let n=kt();n&&t.set("X-CSRF-Token",n);let a=await fetch("/api/v1/me/avatar",{method:"PUT",credentials:"same-origin",headers:t,body:e}),r=await a.json().catch(()=>({}));if(!a.ok)throw new Error(r.error||a.statusText||"Avatar upload failed");return r}function wa(e){N.set({...N.get(),user:e})}function ir({user:e}){let[t,n]=b(!1);async function a(r){if(r.preventDefault(),t)return;n(!0);let s=new FormData(r.currentTarget);try{let i=await w("/api/v1/me/profile",{method:"PATCH",body:{display_name:ya(s.get("display_name")),bio:ya(s.get("bio"))}});wa(i),k("Profile saved.")}catch(i){k(i.message)}finally{n(!1)}}return o`<form class="profile-form" onSubmit=${a}>
  ...
  </form>`}function lr({user:e}){let[t,n]=b(!1);async function a(r){if(r.preventDefault(),t)return;let s=r.currentTarget.elements.avatar?.files?.[0];if(!s){k("Choose an avatar image first.");return}n(!0);try{let i=await or(s);wa(i),k("Avatar uploaded.")}catch(i){k(i.message)}finally{n(!1)}}return o`<form class="profile-form" onSubmit=${a}>
  ...
  </main>`:null}X();async function cr(){let[e,t]=await Promise.all([w("/api/v1/auth/sessions"),w("/api/v1/auth/device-tokens")]);return{sessions:e,deviceTokens:t}}function ur({item:e,onRevoke:t}){return o`<div class="management-item">
    <div>
  ...
  </main>`}var pr={publicLibrary:nt,publicGame:nt,games:Hn,library:Zn,clip:Dt,public:Dt,login:da,resetPassword:pa,admin:ga,profile:ka,account:Sa,publicUser:xa,about:Ta},Pa=En({pathname:window.location.pathname,search:window.location.search});function mr(){let e=Dn();Pa=e.name;let{ready:t,user:n}=q(N),a=t&&Pn(e.name,n);if(x(()=>{a&&G("/login")},[a]),!t||a)return o`<div class="boot">Loading…</div>`;let r=pr[e.name]||nt,s=e.name==="login"||e.name==="resetPassword";return o`<div class="ui" onClick=${Un}>
